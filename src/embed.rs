...
pub async fn run(pool: &PgPool, args: EmbedCmd) -> Result<()> {
    ...
    loop {
        ...
        let rows = fetch_chunks(pool, &model_tag, args.force, n).await?;
        ...
        // insert embeddings
        // bind is used, because no vector is not understood at compile time 
...
fn build_session(onnx_path: &std::path::Path, device: Device) -> Result<Session> {
    ...
    let builder = match device {
        ...
        Device::Cuda => {
            #[cfg(feature = "cuda")]
            {
                use ort::session::builder::CUDAExecutionProvider;
